/** Static assets under public/images */
export const images = {
  heroWebDesign: {
    mobile: "/images/hero/web-design/mobile.png",
    tablet: "/images/hero/web-design/tablet.png",
    desktop: "/images/hero/web-design/desktop.png",
  },
  heroWebDevelopment: {
    mobile: "/images/hero/web-development/mobile.png",
    tablet: "/images/hero/web-development/tablet.png",
    desktop: "/images/hero/web-development/desktop.png",
  },
  heroPhotography: {
    mobile: "/images/hero/photography/mobile.png",
    tablet: "/images/hero/photography/tablet.png",
    desktop: "/images/hero/photography/desktop.png",
  },
  webSubHero: {
    mobile: "/images/web-sub-hero/mobile.png",
    tablet: "/images/web-sub-hero/tablet.png",
    desktop: "/images/web-sub-hero/desktop.png",
  },
  photoSubHero: {
    mobile: "/images/photo-sub-hero/mobile.png",
    tablet: "/images/photo-sub-hero/tablet.png",
    desktop: "/images/photo-sub-hero/desktop.png",
  },
  contactPhoto: {
    desktop: "/images/contact/desktop.png",
  },
  icons: {
    hamburger: "/images/icons/hamburger.svg",
    menuClose: "/images/icons/menu-close.svg",
    heroArrowMobile: "/images/icons/hero-arrow-mobile.svg",
    heroArrowTablet: "/images/icons/hero-arrow-tablet.svg",
    heroArrowDesktop: "/images/icons/hero-arrow-desktop.svg",
    sectionHighlight: "/images/icons/section-highlight.svg",
    viewAllArrow: "/images/icons/view-all-arrow.svg",
    pillLinkArrowRight: "/images/icons/pill-link-arrow-right.svg",
    contactSubmitArrow: "/images/icons/contact-submit-arrow.svg",
    linkedinFooter: "/images/icons/linkedin-footer.svg",
    githubFooter: "/images/icons/github-footer.svg",
    breadcrumbSeparator: "/images/icons/breadcrumb-separator.svg",
    homeLinkArrow: "/images/icons/home-link-arrow.svg",
    modalClose: "/images/icons/modal-close.svg",
    modalArrowPrev: "/images/icons/modal-arrow-prev.svg",
    modalArrowNext: "/images/icons/modal-arrow-next.svg",
  },
} as const;
